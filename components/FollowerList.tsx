...
export function FollowerList({ data }: { data?: FollowerData[] }) {
    ...
    // If data is provided (even empty), use it. Only fallback if data is undefined (loading state handled mainly by parent, but here we treat undefined as 'not yet loaded' or 'no data passed')
    const hasData = data && Array.isArray(data);

    const displayData = hasData ? data.map(u => ({
